...
The data provided here should be considered
...
with(subset(LakeO.xtab,DATE==YEST),points(DOWY,LOK.stage,pch=21,bg=adjustcolor("red",0.5),col="red",lwd=0.1,cex=1.25))
with(subset(LakeO.xtab,DATE==YEST),segments(DOWY,LOK.stage,DOWY,10.1,lty=2))
with(subset(LakeO.xtab,DATE==YEST),text(DOWY,10,paste(format(round(LOK.stage,2),nsmall=2),"Ft"),cex=0.8,xpd=NA))
}
...
```{r LOK_Q, echo=FALSE,fig.width=6,fig.height=4,fig.align='center',fig.cap="Lake Okeechobee inflow and outflow volume from USACE."}
...
and **`r format(round(EAAq.pos,0),big.mark=",")` AF** to the
...
```{r WCA1_Q, echo=FALSE,fig.width=6,fig.height=4,fig.align='center',fig.cap="WCA1 inflow and outflow volume from USACE."}
...
max.q=max(subset(map.q,Date%in%seq(End.Date-ddays(60),End.Date+ddays(3),"1 days"))$WCA1.kacft,na.rm=T)
ylim.max=round(max.q+max.q*0.25,-1)
...
```{r WCA2_Q, echo=FALSE,fig.width=6,fig.height=4,fig.align='center',fig.cap="WCA2 inflow and outflow volume from USACE."}
...
max.q=max(subset(map.q,Date%in%seq(End.Date-ddays(60),End.Date+ddays(3),"1 days"))$WCA2.kacft,na.rm=T)
ylim.max=round(max.q+max.q*0.25,-1)
...
```{r WCA3_Q, echo=FALSE,fig.width=6,fig.height=4,fig.align='center',fig.cap="WCA3A inflow and outflow volume from USACE."}

map.q$WCA3.kacft=cfs.to.acftd(map.q$WCA2)/1000
...
```{r SRS_Q, echo=FALSE,fig.width=6,fig.height=4,fig.align='center',fig.cap="WCA3A inflow and outflow volume from USACE."}
...
legend(0.5,0.5,
       ...
       col=cols,lty=c(1,2,3),lwd=c(2.5,2.5),pt.cex=2.5,ncol=1,cex=1,bty="n",y.intersp=1,x.intersp=0.5,xpd=NA,xjust=0.5)

```

<br>

```{r SRS_cumQ, echo=FALSE,fig.width=6.5,fig.height=4,fig.align='center',fig.cap="Cumulative discharge for Shark River Slough for the last three Federal Water Years (Oct - Sept). Day of water year is analogous to day of year with Day 1 = October 1."}
...
mtext("Day of Federal Water Year",side=1,line=2,cex=1.25)
...
CRE Discharge Salinity Envelope - 6 months [plot](https://github.com/SwampThingPaul/EvergladesConditions/blob/4676ac1d524b35a44ea6e6991a07fad9b5fe6bc3/report/plots/CREFlow_6month.png)

**Caloosahatchee Conditions Summary:** Flow to the Caloosahatchee
Estuary had a 7-day average of **`r format(round(subset(cal.flow.dat.xtab,Date==date.fun(End.Date-ddays(1)))$S79Q.7,0),big.mark=",")`
cfs** at **S-79** with a 7-day average of
**`r format(round(subset(cal.flow.dat.xtab,Date==date.fun(End.Date-ddays(1)))$S77Q.7,0),big.mark=",")` cfs coming from the lake
...
```{r CRE_LOK_Q, echo=FALSE,fig.width=7,fig.height=4,fig.align='center',fig.cap="S-77 (from Lake Okeechobee) and S-79 (to Tidal Basin) flow data along the C-43 and Caloosahatchee River. C-43 Basin Flow was calculated as the difference from S-77 and S-79."}
...
```{r CRE_cumQ, echo=FALSE,fig.width=7,fig.height=4,fig.align='center',fig.cap="Cumulative discharge for S-79 (to Tidal Basin) and S-77 (from Lake Okeechobee) for the last three Florida Water Years (May - April). Day of water year is analogous to day of year with Day 1 = May 1."}
...
CRE Salinity and Discharge - 6 months [plot](https://github.com/SwampThingPaul/EvergladesConditions/blob/4676ac1d524b35a44ea6e6991a07fad9b5fe6bc3/report/plots/CRESalPlot.png)
...
**St Lucie Conditions Summary:** Flow to the St Lucie
Estuary had a 7-day average of **`r format(round(subset(SLE.q.dat,DATE==date.fun(End.Date-ddays(1)))$Q.7,0),big.mark=",")`
cfs** from **North and South Forks** with a 7-day average of
**`r format(round(subset(SLE.q.dat,DATE==date.fun(End.Date-ddays(1)))$S308Q.7,0),big.mark=",")` cfs** coming from the lake
at **S-308**. 

```{r SLE_LOK_Q, echo=FALSE,fig.width=6,fig.height=4,fig.align='center',fig.cap="S-308 (from Lake Okeechobee), S-80 (to Lower Fork),  C-44 Basin and North Fork (Gordy Rd, S49, S48) daily discharge. C-44 Basin Flow was calculated as the difference from S-308 and S-80."}
...
ylim.val=c(0,ylim.max);by.y=ylim.max/4;ymaj=seq(ylim.val[1],ylim.val[2],by.y);ymin=seq(ylim.val[1],ylim.val[2],by.y/2)
xlim.val=date.fun(c(End.Date-ddays(30),End.Date+ddays(3)));xmaj=seq(xlim.val[1],xlim.val[2],by="7 days");xmin=seq(xlim.val[1],xlim.val[2],by="1 days")
cols=c("dodgerblue1","indianred1","black","grey50")
...
with(SLE.q.dat,lines(DATE,NorthFork,col=cols[4],lwd=2,lty=3))
axis_fun(1,xmaj,xmin,format(xmaj,"%m/%d/%Y"),line=-0.5)
axis_fun(2,ymaj,ymin,format(ymaj),cex.axis=1)
box(lwd=lwd.val)
...
legend(0.5,0.6,
       ...
       col=cols,lty=c(1,2,3,3),lwd=c(2,2,2,2),pt.cex=2,ncol=2,cex=1,bty="n",y.intersp=1,x.intersp=0.5,xpd=NA,xjust=0.5)

```

<br>

```{r SLE_cumQ, echo=FALSE,fig.width=7,fig.height=4,fig.align='center',fig.cap="Cumulative discharge for S-80 (to Tidal Basin) and S-308 (from Lake Okeechobee) for the last three Florida Water Years (May - April). Day of water year is analogous to day of year with Day 1 = May 1."}

...
# End.Date=date.fun(Sys.time())
flab.dates=c(date.fun(paste(CurWY-10,05,01,sep="-")),End.Date)
...
for(i in 1:nrow(comp.dbkey)){
  tmp=DBHYDRO_daily(dates[1],dates[2],comp.dbkey$DBKEY[i],3)
...
for(i in 1:nrow(WCA1.dbkeys)){
  tmp=DBHYDRO_daily(dates[1],dates[2],WCA1.dbkeys$DBKEY[i],3)
...
for(i in 1:nrow(WCA2.dbkeys)){
  tmp=DBHYDRO_daily(dates[1],dates[2],WCA2.dbkeys$DBKEY[i],3)
...
for(i in 1:nrow(WCA3.dbkeys)){
  tmp=DBHYDRO_daily(dates[1],dates[2],WCA3.dbkeys$DBKEY[i],3)
...
for(i in 1:nrow(ENP.dbkeys)){
  tmp=DBHYDRO_daily(dates[1],dates[2],ENP.dbkeys$DBKEY[i],3)
...
for(i in 1:nrow(SRS.dbkeys)){
  tmp=DBHYDRO_daily(flab.dates[1],flab.dates[2],SRS.dbkeys$DBKEY[i],3)
...
### SRS inflows
srs.flow.da.xtab$SRS.east=rowSums(cfs.to.acftd(srs.flow.da.xtab[,c("S12A","S12B","S12C","S12D")]),na.rm=T)/1000
srs.flow.da.xtab$SRS.west=rowSums(cfs.to.acftd(srs.flow.da.xtab[,c("S333","S333N","S355A","S355B","minS356_S335")]),na.rm=T)/1000
...
for(i in 1:nrow(FLAB.sal.dbkeys)){
  tmp=DBHYDRO_daily(flab.dates[1],flab.dates[2],FLAB.sal.dbkeys$DBKEY[i],3)
...
for(i in 1:nrow(cal.flow)){
  tmp=DBHYDRO_daily(date.fun(paste(CurWY-3,"05-01",sep="-")),End.Date,cal.flow$DBKEY[i],3)
...
for(i in 1:nrow(SLE.dbkeys)){
  tmp=DBHYDRO_daily(dates[1],dates[2],SLE.dbkeys$DBKEY[i],3)
...
for(i in 1:nrow(LWL.dbkeys)){
  tmp=DBHYDRO_daily(dates[1],dates[2],LWL.dbkeys$DBKEY[i],3)
...
if(subset(LakeO.xtab,DATE==YEST)$DOWY<300){
  ...
  with(subset(LakeO.xtab,DATE==YEST),text(DOWY,10,paste(format(round(LOK.stage,2),nsmall=2),"Ft\nNGVD29"),cex=0.8,xpd=NA,pos=1,offset=-0.1))
}else{
    with(subset(LakeO.xtab,DATE==YEST),segments(DOWY,LOK.stage,DOWY-30,10.1,lty=2))
  with(subset(LakeO.xtab,DATE==YEST),text(DOWY-30,10,paste(format(round(LOK.stage,2),nsmall=2),"Ft\nNGVD29"),cex=0.8,xpd=NA,pos=1,offset=-0.1))
  }
...
site.val=c("SRS.east","SRS.west")
site.val.lab=c("SRS East\n(S12s)","SRS West\n(S333s, S335s & min(S356, S335))")
#i=1
...
max.q=with(subset(SLE.q.dat,DATE%in%seq(End.Date-ddays(60),End.Date+ddays(3),"1 days")),max(c(NorthFork,S80,S308),na.rm=T))#
...
# LWL.q.dat
max.q=max(subset(LWL.q.dat,DATE%in%seq(End.Date-ddays(30),End.Date+ddays(3),"1 days"))$TFlow.cfs,na.rm=T)
ylim.max=round(max.q+max.q*0.25,-3)
...
Estuary had a 7-day average of **`r format(round(subset(LWL.q.dat,DATE==date.fun(End.Date-ddays(1)))$Q.7,0),big.mark=",")`
cfs**, with  **`r format(round(subset(LWL.q.dat,DATE==date.fun(End.Date-ddays(1)))$S44.Q.7,0),big.mark=",")`
cfs** from S44, **`r format(round(subset(LWL.q.dat,DATE==date.fun(End.Date-ddays(1)))$S155.Q.7,0),big.mark=",")`
cfs** from S155 and **`r format(round(subset(LWL.q.dat,DATE==date.fun(End.Date-ddays(1)))$S41.Q.7,0),big.mark=",")`
cfs** from S41. 

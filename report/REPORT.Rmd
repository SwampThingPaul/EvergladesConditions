...
usgs_sites$monitoring_location_id <- paste0("USGS-",usgs_sites$site_no)

loc.dat.shp <- read_waterdata_monitoring_location(usgs_sites$monitoring_location_id )|>
  st_transform(utm17)|>
  merge(usgs_sites,"monitoring_location_id")
...
# usgs_sites$monitoring_location_id <- paste0("USGS-",usgs_sites$site_no)
## Creek Discharge 
flab_crk_list <- future_lapply(seq_along(usgs_sites$monitoring_location_id), function(i) {
  # Attempt to retrieve data and handle errors
  tryCatch({
    site_id <- usgs_sites$monitoring_location_id[i]
    sdate <- flab.dates[1]
    edate <- dates[2]
    
    # tmp <- readNWISdv(site_id,pCode,sdate,edate)|>renameNWISColumns()
...
Qdat_flab_crk <- do.call(rbind, flab_crk_list)|>
  merge(usgs_sites,"monitoring_location_id",all.x=T)|>
  merge(pCode_def,"parameter_code")|>
  mutate(Date = date.fun(time))

Qdat_flab_crk1 <- Qdat_flab_crk
Qdat_flab_crk <- dcast(Qdat_flab_crk,site_no+loc+loc_abb+Date~param,value.var = "value",mean)

Qdat_flab_crk$Flow <- with(Qdat_flab_crk,ifelse(is.na(Q),Q_tidefilt,Q))
...
flab_crk_uv_list <- future_lapply(seq_along(uv_sites$monitoring_location_id), function(i) {
  # Attempt to retrieve data and handle errors
  tryCatch({
    site_id <- uv_sites$monitoring_location_id[i]
...
Qdat_flab_crk_da <- dcast(Qdat_flab_crk_uv,site_no+loc+loc_abb+Date~param,value.var = "value",mean)
Qdat_flab_crk_da$Flow <- with(Qdat_flab_crk_da,ifelse(is.na(Q),Q_tidefilt,Q))
...
# read_waterdata_monitoring_location(paste0("USGS-",sites))
# readNWISsite(sites)
SLE_sal_dat_list <- future_lapply(seq_along(sites), function(i) {
  # Attempt to retrieve data and handle errors
  tryCatch({
    site_id <- paste0("USGS-",sites[i])
    sdate <- Start.Date[1]
    edate <- YEST
    
    tmp <- read_waterdata_daily(
      monitoring_location_id = site_id,
      parameter_code = pCode2,
      time = c(sdate, edate),
      skipGeometry = TRUE
    )
...
SLE_sal_dat <- do.call(rbind,SLE_sal_dat_list)|>
  merge(data.frame(monitoring_location_id = paste0("USGS-",sites),
                   SITE = c("STL_RIVER","STL_STPT")),
        "monitoring_location_id")|>
  merge(
    data.frame(parameter_code = pCode2,
               param = c("WT","SPC","SAL")),
    "parameter_code"
  )
...
SLE_sal_dat$Date <- date.fun(SLE_sal_dat$time)

SLE_sal_dat_xtab <- dcast(SLE_sal_dat,SITE+Date~param,value.var = "value",mean,na.rm=TRUE)
...
EAA_Q.xtab <- dcast(EAA_Q,Date.EST+WY~loc,value.var = "Data.Value",
                    fun.aggregate = function(x) mean(cfs.to.acftd(x)/1000,na.rm=T))
...
EAA_Q.xtab.sum <-  melt(EAA_Q.xtab[,c("Date.EST",vars)],id.vars="Date.EST")|>
  aggregate(value~Date.EST+variable,FUN = function(x) round(sum(x,na.rm=T),2))|>
  rename(c("value"="TFlow"))
...
EAA_Q.xtab.sum$WY <- WY(EAA_Q.xtab.sum$Date.EST)
# subset(EAA_Q.xtab.sum,WY==2025&Basin == "S5A")|>
#   dcast(Basin+region~direct,value.var = "TFlow",fun.aggregate = function(x) sum(x,na.rm=T)/1000)



EAA_Q.xtab.sum.direct <- subset(EAA_Q.xtab.sum,direct%in%c("Inflow","Outflow","Pass"))|>
  dcast(Date.EST~direct,value.var = "TFlow",fun.aggregate = function(x) sum(x,na.rm=T));# calculates total kacft
# tail(EAA_Q.xtab.sum.direct)

EAA_Q.xtab.sum.direct$month <- as.numeric(format(EAA_Q.xtab.sum.direct$Date.EST,"%m"))
EAA_Q.xtab.sum.direct$CY <- as.numeric(format(EAA_Q.xtab.sum.direct$Date.EST,"%Y"))
EAA_Q.xtab.sum.direct$WY <- WY(EAA_Q.xtab.sum.direct$Date.EST)
# tail(EAA_Q.xtab.sum.direct)
tmp <- subset(EAA_Q.xtab.sum.direct,CY==2026)|>
  ddply(c("CY","month"),summarise,
        TInflow = sum(cfs.to.acftd(Inflow)),
        TOutflow = sum(cfs.to.acftd(Outflow)),
        TPass = sum(cfs.to.acftd(Pass)))

# subset(EAA_Q.xtab.sum.direct,WY==2025)|>
#   ddply(c("WY"),summarise,
#         TInflow = sum(Inflow/1000),
#         TOutflow = sum(Outflow/1000),
#         TPass = sum(Pass/1000))

```

```{r EAA_Q plot,echo=FALSE,fig.width=6.5,fig.height=4.5,fig.align='center',fig.cap="Lake Okeechobee inflow and outflow volume from DBHYDRO",include = FALSE}
...
```{r STA_Q plot,echo=FALSE,fig.width=6.5,fig.height=4.5,fig.align='center',fig.cap="Everglades STA Inflow from the Lake and Basin.",include = FALSE}
...
ylim.val.min=c(9,9,9,8)
ylim.val.max=c(13,13,16,13)
for(i in seq_along(eaa_canal_labs)){
  ...
  ylim.val <- c(ylim.val.min[i],ylim.val.max[i])
...
mean_cols <- c("CA1-8T","CA1-9","CA1-7")
missing_cols <- setdiff(mean_cols, names(WCA1.xtab))
if (length(missing_cols) > 0L) {
  for (cc in missing_cols) {
    WCA1.xtab[[cc]] <- NA_real_
  }
}

WCA1.xtab$Mean <- apply(WCA1.xtab[, mean_cols, drop = FALSE],1,
  function(x) {
    x <- mean(x,na.rm=T)
    if (length(x) == 0L) NA_real_ else x[1L]
  }
...
```{r WCA3plot,echo=FALSE,fig.width=7,fig.height=5.5,fig.align='center',fig.cap=paste0("Water Conservation Area 3A (WCA3A) daily average stage elevation for water year ",CurWY," (WY", CurWY,") relative to the last two water years relative to the WCA3A Regulation Schedule. Average stage values represented by the 3-gauge average (3-63,3-64,3-65) and 2-gauge average (3-62 and 3-63).")}
...
mean_cols <- c("CA3-65","CA3-64","CA3-63")
missing_cols <- setdiff(mean_cols, names(WCA3.xtab))
if (length(missing_cols) > 0L) {
  for (cc in missing_cols) {
    WCA3.xtab[[cc]] <- NA_real_
  }
}
WCA3.xtab$Avg.636465 <- apply(WCA3.xtab[, mean_cols, drop = FALSE],1,
  function(x) {
    x <- mean(x,na.rm=T)
    if (length(x) == 0L) NA_real_ else x[1L]
  }
...
WCA3.xtab$Avg.6263 <- apply(WCA3.xtab[, mean_cols, drop = FALSE],1,
  function(x) {
    x <- mean(x,na.rm=T)
    if (length(x) == 0L) NA_real_ else x[1L]
  }
...
plot(0:1,0:1,type="n",yaxt="n",xaxt="n",bty="n")
legend(0.5,0.75,
       legend=c("Inflow","Outflow","S12s+S333s"),
       col=c(Q_cols[1:2],"black"),lty=c(1,2,2),lwd=c(2.5,2.5),pt.cex=2.5,ncol=3,cex=1,bty="n",y.intersp=1,x.intersp=0.5,xpd=NA,xjust=0.5)
...
# wca.Q.da.mon2$monCY.date <- with(wca.Q.da.mon2,date.fun(paste(CY,month,"01",sep="-")))
# date_fill <- data.frame(monCY.date = seq(min(wca.Q.da.mon2$monCY.date),
#                                          date.fun(paste(max(wca.Q.da.mon2$CY),12,"01",sep="-")),
#                                          "1 month")
# )|>
#   mutate(month = as.numeric(format(monCY.date,"%m")),
#          CY = as.numeric(format(monCY.date,"%Y")))
# wca.Q.da.mon2 <- merge(wca.Q.da.mon2,date_fill,c("CY","month","monCY.date"),all.y = TRUE,sort = FALSE)
...
xmaj.x <- which(wca.Q.da.mon2$month == 1)
xmaj.lab <- wca.Q.da.mon2$CY[xmaj.x]
...
mean_cols <- c("S333","S334")
missing_cols <- setdiff(mean_cols, names(L29_stg_xtab))
if (length(missing_cols) > 0L) {
  for (cc in missing_cols) {
    L29_stg_xtab[[cc]] <- NA_real_
  }
}
L29_stg_xtab$L29 <- apply(L29_stg_xtab[, mean_cols, drop = FALSE],1,
  function(x) {
    x <- mean(x,na.rm=T)
    if (length(x) == 0L) NA_real_ else x[1L]
  }
...
ENP.xtab <- subset(stg_dat,loc%in%c("NESRS2"))|>
...
mtext(side=1,"Month",line=2,cex=1.25)
mtext(side=2,"Stage Elevation (Feet, NGVD29)",line=2.5,cex=1)
mtext(side=3,adj=0,paste("Date:",format(date.fun(as.Date(Sys.Date())),"%b %d, %Y")),cex=0.8)
...
ylim.val <- c(5,10)# ;by.y=0.5;ymaj=seq(ylim.val[1],ylim.val[2],by.y);ymin=seq(ylim.val[1],ylim.val[2],by.y/2)
...
cols.vals <- stage_cols# rep(rev(stage_cols),2)
lty.vals <- 1 # c(rep(1,length(stage_yrs)),rep(2,length(stage_yrs)))
lwd.vals <- c(1,1,1)
plot(0:1,0:1,type = 'n', axes = F,xlab=NA, ylab=NA)
legend(0.5,0.5,
       legend=labs.val,
...
for(i in 1:length(site.val)){
  tmp.dat <- subset(ENP_Q,Date%in%seq(date.fun(as.Date(Sys.Date())-100),date.fun(as.Date(Sys.Date())),"1 days"))[,c("Date","WY","DOWY","SRS_east","SRS_west")]|>
    melt(id.vars=c("Date","WY","DOWY"),variable.name = "loc",value.name = "flow.acft")|>
  ...
  tmp.dat <- merge(tmp.dat,ENP_Q_stats,c("DOWY","loc"))
  tmp.dat <- tmp.dat[order(tmp.dat$Date),]
  plot(flow.acft~Date,tmp.dat,type="n",ylim=ylim.val,xlim=xlim.val,axes=F,ann=F)
  with(subset(tmp.dat,loc==site.val[i]),shaded.range(Date,q10,q90,"lightblue",lty=1,col.adj=1))
  lines(q50~Date,subset(tmp.dat,loc==site.val[i]),col="grey50")
  with(tmp.dat,pt_line(Date,flow.acft,2,"black",1,21,"indianred1",cex=1.25))
  abline(h=ymaj,v=hydro.day(xmaj),lty=3,col="grey")
  axis_fun(1,xmaj,xmin,format(xmaj,"%m-%d"),line=-0.5)
  axis_fun(2,ymaj,ymin,format(ymaj/1000),cex.axis=1);box(lwd=1)
...
xmaj.x <- which(SRS_del$month == 1)
xmaj.lab <- SRS_del$CY[xmaj.x]
...
leg.fun(b = bks,
        pal = sal_delta_cols,
        expression(Delta*"(30-day Avg Salinity, 30 PSU)"),
        x.max=0.6,x.min=0.4,bot.val = 0.4,leg.type = "continuous")
legend(0.5,0.2,legend=c("> 30 PSU","< 30 PSU"),
       pch=c(24,25),pt.bg=c("grey"),
       lty=c(0,0),lwd=c(0.5,0.5),col="black",
       pt.cex=1.5,cex=1,ncol=1,bty="n",y.intersp=1.25,x.intersp=0.75,xpd=NA,xjust=0.5,yjust=0.5)

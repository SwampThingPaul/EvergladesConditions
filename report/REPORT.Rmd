...
# )
# 
# map.q[,!(names(map.q) %in% "Date")]=sapply(map.q[,!(names(map.q) %in% "Date")], FUN=function(x) ifelse(is.na(x)==T,0,x))
...
map.q <- read.csv("USACE_20d.csv")
# map.q <- read.csv("./report/USACE_20d.csv")
...
q.tab[,vars]%>%
  ...
  flextable::align(j=2:14,part="all",align="center")%>%
...
# plot(ZONE.B~Date,subset(LORS08.sch.bk,as.numeric(format(Date,'%Y'))==2023),ylim=c(9,18),type="l")
# lines(ZONE.A~Date,subset(LOSOM.sch.bk,as.numeric(format(Date,'%Y'))==2023))
...
with(subset(LakeO.xtab,WY==CurWY-3),lines(DOWY,LOK.stage,lwd=4,col=adjustcolor("purple",0.5),lty=1))
...
if(subset(LakeO.xtab,DATE==YEST)$DOWY>300){
  ...
  with(subset(LakeO.xtab,DATE==YEST),text(DOWY-30,lab.loc,paste(format(round(LOK.stage,2),nsmall=2),"Ft\nNGVD29"),cex=0.8,xpd=NA,pos=1,offset=-0.1))
...
legend(0.5,0.5,
       legend=c(paste0("WY",CurWY),paste0("WY",CurWY-1),paste0("WY",CurWY-2),paste0("WY",CurWY-3)),
       col=c("red",adjustcolor(c("blue","forestgreen","purple"),0.5)),lty=c(1,1),lwd=c(4,4,4),ncol=2,cex=1,bty="n",y.intersp=1,x.intersp=0.5,xpd=NA,xjust=0.5)

# subset(LOSOM.sch.bk,Date==YEST)
# subset(LakeO.xtab,DATE==YEST)
```
...
subset(LakeO.xtab,DOWY==hydro.day(YEST)&WY%in%(CurWY+c(0,-1:-3)))%>%
...
```{r LOK RecoveryOPS, echo=FALSE,fig.width = 6,fig.height=5,fig.align='center',fig.cap="Tracking Lake Okeechobee Recovery Operations targets."}
if(rec.ops ==  1){
par(family="serif",mar=c(3,1,1,0.5),oma=c(2,3,1,0.5));

xlim.vals <- c(date.fun("2024-12-01"),YEST+(30*86400))
xmaj <- seq(xlim.vals[1],xlim.vals[2],by="1 months");xmin <- seq(xlim.vals[1],xlim.vals[2],by="1 days")

ylim.val=c(0,180);by.y=30
ymaj=seq(ylim.val[1],ylim.val[2],by.y);ymin=seq(ylim.val[1],ylim.val[2],by.y/2)

plot(cum.LT12.cnt~DATE,LOK.rec.stg,ylim=ylim.val,xlim=xlim.vals,type="n",ann=F,axes=F);
abline(h=ymaj,v=xmaj,lty=3,col="grey",lwd=0.5)
lines(cum.LT12.cnt~DATE,LOK.rec.stg,col="dodgerblue1",lwd=2)
points(cum.LT12.cnt~DATE,subset(LOK.rec.stg,DATE==YEST),pch=21,bg="red",col="pink",cex=1.2)
with(subset(LOK.rec.stg,DATE==YEST),text(DATE,cum.LT12.cnt,cum.LT12.cnt,pos=4,offset=0.25))
abline(h=90,col="dodgerblue1",lty=2,lwd=2)
text(lake.rec.start+(11*86400),90,"Days < 12.0 Ft Target",cex=0.75,pos=1,offset=0.25)

lines(cum.LT115.cnt~DATE,LOK.rec.stg,col="orange",lwd=2)
points(cum.LT115.cnt~DATE,subset(LOK.rec.stg,DATE==YEST),pch=21,bg="red",col="pink",cex=1.2)
with(subset(LOK.rec.stg,DATE==YEST),text(DATE,cum.LT115.cnt,cum.LT115.cnt,pos=4,offset=0.25))
abline(h=60,col="orange",lty=2,lwd=2)
text(lake.rec.start+(11*86400),60,"Days < 11.5 Ft Target",cex=0.75,pos=1,offset=0.25)
axis_fun(1,xmaj,xmin,format(xmaj,"%b\n%Y"),line=0.5)
axis_fun(2,ymaj,ymin,ymaj);box(lwd=1)
mtext(side=2,line=2.5,"Cumulative Days")
mtext(side=1,line=3,"Date (Month-Year)")
mtext(side=3,adj=0,paste0("Since Recovery Operations (",format(lake.rec.start,"%d %b %Y"),")"))

legend("topleft",
       legend=c("Days < 12.0 Ft","Days <11.5 Ft"),
       col=c("dodgerblue1","orange"),lty=c(1),lwd=c(2),pt.cex=1,ncol=1,cex=1,bty="n",y.intersp=1,x.intersp=0.5,xpd=NA,xjust=0.5)

  # cat(sprintf("<center>Tracking Lake Okeechobee Recovery Operations targets.</center>", i))
}
```
...
cols <- hcl.colors(8,palette="Spectral")
ggplot(LakeO.xtab, aes(x = DOWY, y = WY, fill = stg.cat)) +
  geom_tile(aes(group = stg.cat), colour = NA)+
  scale_y_reverse(expand = c(0, 0), breaks = unique(LakeO.xtab$WY)) +
...
# link_txt <- link_txt[!grepl(".zip",link_txt$name), , drop = FALSE]
...
# noaa.image.inventory2=subset(noaa.image.inventory2,date>max(cyano_area$date))
## data download  ------------------------------------------------
...
# 
# ## update for missing data
# local.image.inventory2_1=subset(local.image.inventory2,date>max(cyano_area$date))
# 
# if(max(cyano_area$date) == max(local.image.inventory2$date,na.rm=T)){
# curr.dat=subset(local.image.inventory2,date==max(local.image.inventory2$date,na.rm=T))
#   tc.raster = raster::raster(paste(data.path,"truecolor", curr.dat$fname,sep="/"))
#   tc.raster = raster::RGB(tc.raster)
# }
...
subset(EAA_canals,DOWY==hydro.day(YEST))[,c("DATE","WY",site.vals)]%>%
  flextable()%>%
  flextable::width(width=c(1,0.75,rep(0.8,5)))%>%
  flextable::align(align="center",part="all")%>%
  colformat_datetime(j=1,fmt_date="%m-%d")%>%
  colformat_double(j=2,big.mark="",digits=0)%>%
  colformat_num(j=3:5,na_str="- NR -")%>%
...
ymaj=heckbert_labs(min(map.q$WCA2.kacft,na.rm=T),max(map.q$WCA2.kacft,na.rm=T),5)#seq(ylim.val[1],ylim.val[2],by.y);
ymin=seq(ylim.val[1],ylim.val[2],mean(diff(ymaj))/2)# seq(ylim.val[1],ylim.val[2],by.y/2)
ylim.val=range(ymaj)# c(0,ylim.max);by.y=ylim.max/4;
...
wca.Q.da.mon2 <- ddply(wca.Q.da,c("REGION","CY","month"),summarise,TFlow.acft =sum(cfs.to.acftd(inflow.Q)/1000,na.rm=T))
wca.Q.da.mon2 <- dcast(subset(wca.Q.da,CY>2011),CY+month~REGION,value.var="inflow.Q",fun.aggregate = function(x) sum(cfs.to.acftd(x)/1000,na.rm=T))

ylim.val=c(0,300);# by.y=100;ymaj=seq(ylim.val[1],ylim.val[2],by.y);ymin=seq(ylim.val[1],ylim.val[2],by.y/2)
ymaj <- labeling::heckbert(ylim.val[1],ylim.val[2],5)
...
plot(L29~DOWY,L29.stg.da.loc.xtab,ylim=ylim.val,xlim=xlim.vals2,type="n",ann=F,axes=F,yaxs="i",xaxs="i")
abline(h=ymaj,v=hydro.day(xmin),lwd=1,col="grey",lty=3)
lines(L29~DOWY,subset(L29.stg.da.loc.xtab,WY==CurWY-2),lwd=2,col=adjustcolor("forestgreen",0.5),lty=1)
lines(L29~DOWY,subset(L29.stg.da.loc.xtab,WY==CurWY-1),lwd=2,col=adjustcolor("blue",0.5),lty=1)
lines(L29~DOWY,subset(L29.stg.da.loc.xtab,WY==CurWY),lwd=2,col="red",lty=1)

# if(is.na(subset(ENP.xtab,DATE==YEST)$L29Canal.f)==T|nrow(subset(ENP.xtab,DATE==YEST))==0){NA}else{
if(nrow(subset(L29.stg.da.loc.xtab,Date.EST==YEST))>0){
  with(subset(L29.stg.da.loc.xtab,Date.EST==YEST),points(DOWY,L29,pch=21,bg=adjustcolor("red",0.5),col="red",lwd=0.1,cex=1.25))
  ...
  if(subset(L29.stg.da.loc.xtab,Date.EST==YEST)$DOWY>300){
    with(subset(L29.stg.da.loc.xtab,Date.EST==YEST),segments(DOWY,L29,DOWY-30,6.7,lty=2))
    with(subset(L29.stg.da.loc.xtab,Date.EST==YEST),text(DOWY-30,6.6,paste(format(round(L29,2),nsmall=2),"Ft\nNGVD29"),cex=0.8,xpd=NA,pos=1,offset=-0.1))
  }else if(subset(L29.stg.da.loc.xtab,Date.EST==YEST)$DOWY<30){
    with(subset(L29.stg.da.loc.xtab,Date.EST==YEST),segments(DOWY,L29,DOWY+30,6.7,lty=2))
    with(subset(L29.stg.da.loc.xtab,Date.EST==YEST),text(DOWY+30,6.6,paste(format(round(L29,2),nsmall=2),"Ft\nNGVD29"),cex=0.8,xpd=NA,pos=1,offset=0.1))
  }else{
    with(subset(L29.stg.da.loc.xtab,Date.EST==YEST),segments(DOWY,L29,DOWY,6.7,lty=2))
    with(subset(L29.stg.da.loc.xtab,Date.EST==YEST),text(DOWY,6.6,paste(format(round(L29,2),nsmall=2),"Ft\nNGVD29"),cex=0.8,xpd=NA,pos=1,offset=0.1))
  }
...
for(i in 1:length(site.val)){
  # ylim.val=c(0,max(subset(srs.flow.da.sum,variable==site.val[i])$q90))
  ymaj <- heckbert_labs(0,max(subset(srs.flow.da.sum,variable==site.val[i])$q90),4)
  ymin <- seq(ylim.val[1],ylim.val[2],mean(diff(ymaj))/2)
  ylim.val <- range(ymaj)

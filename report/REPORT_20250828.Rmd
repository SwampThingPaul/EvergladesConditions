...
Qdbkeys3 <- data.frame(loc = c("S10A","S10C","S10D","S94A","G94B","G94C","S39","G338",
...
## Creek Discharge 
flab_crk_list <- lapply(seq_along(usgs_sites$site_no), function(i) {
  # Attempt to retrieve data and handle errors
  tryCatch({
...
Qdat_flab_crk <- merge(Qdat_flab_crk,usgs_sites,"site_no",all.x=T)
Qdat_flab_crk$Date <- date.fun(Qdat_flab_crk$Date)
...
flab_crk_uv_list <- lapply(seq_along(uv_sites$site_no), function(i) {
  # Attempt to retrieve data and handle errors
  tryCatch({
...
Qdat_flab_crk_uv$Date <- date.fun(Qdat_flab_crk_uv$dateTime)
Qdat_flab_crk_da <- ddply(Qdat_flab_crk_uv,c("site_no","agency_cd","Date","loc"),
                          summarise,
                          Flow = mean(Flow_Inst,na.rm=T),
                          Flow_cd = NA)[,names(Qdat_flab_crk)]
...
Qdat_flab_crk <- merge(Qdat_flab_crk,fill,c("loc","site_no","Date"),all.y=T)
...
sal_meta_loc <- data.frame(STATION = c("CCORAL", "ENPBA", "ENPBK", "ENPBN", "ENPBS",
                       ...
                       Y.COORD = c(810605.903, 252062.008, 286306.87, 274043.909, 
                                   ...
                                   782648.064, 859844.95));# changed JBTS to ENPJB

sal_meta_loc_shp <- st_as_sf(sal_meta_loc,coords = c("X.COORD","Y.COORD"),crs = nad83Harn_east)|>
  st_transform(utm17)

sal_dat_list <- lapply(seq_along(sal_dbkeys$DBKEY), function(i) {
  # Attempt to retrieve data and handle errors
  tryCatch({
...
# readNWISsite(sites)
SLE_sal_dat_list <- lapply(seq_along(sites), function(i) {
  # Attempt to retrieve data and handle errors
  tryCatch({
    site_no <- sites[i]
    sdate <- Start.Date[1]
...
#### Creek Discharge
```{r creek_Q, echo=FALSE,fig.width=6.5,fig.height=4.5,fig.align='center',fig.cap="Creek discharge relative to the period of record percentiles."}
...
bbox.lims <- loc.dat.shp|>st_buffer(2500)|>st_bbox()
...
plot(st_geometry(loc_stats_shp),pch=21,bg=percentile_cols[tmp_loc_stats_shp$PercentileBin.f],lwd=0.5,add=T,cex=1.75)
st_txt(subset(loc_stats_shp,loc!="Taylor River (mouth)"),subset(loc_stats_shp,loc!="Taylor River (mouth)")$loc,halo=TRUE,pos=3)
st_txt(subset(loc_stats_shp,loc=="Taylor River (mouth)"),subset(loc_stats_shp,loc=="Taylor River (mouth)")$loc,halo=TRUE,pos=1)
box(lwd=1)
mapmisc::scaleBar(tmp_loc_stats_shp,"bottomleft",bty="n",cex=1,outer=F)
...
```{r FLAB_MFL_Q, echo=FALSE,fig.width=6,fig.height=4.5,fig.align='center',fig.cap="NE Florida Bay net discharge volumes."}
max.q <- max(Qdat_flab_crk_total$total_Q_365,na.rm=T)
ylim.max <- ceiling(max.q*1.1)

ylim.val <- c(0,ylim.max);ymaj <- heckbert_labs(ylim.val[1],ylim.val[2],6);ymin <- seq(ylim.val[1],ylim.val[2],min(diff(ymaj))/2)
xlim.val <- date.fun(c(as.Date(flab.dates[1])+365,as.Date(End.Date)+60));
...
site.val <- paste0("ENP",c("GB","TB","LM","TC","LS"))
...
mtext("Cumulative Discharge (x1000 kAc-Ft)",side=2,line=3.5,cex=1)
...
```{r CRE salinity calendar,echo=FALSE,fig.width=6.5,fig.height=4.5,fig.align='center',fig.cap="Caloosahatchee River Estuary salinity calendar plot." }
sal_cat <- seq(0,40,5)
...
abline(h=ymaj,v=xmaj,lty=3,col="grey")
with(SLE_Q,lines(Date,S80.cfs,col=cols[1],lwd=2))
with(SLE_Q,lines(Date,S308.cfs,col=cols[2],lwd=2,lty=2))
with(SLE_Q,lines(Date,C44,col=cols[3],lwd=2,lty=3))
with(SLE_Q,lines(Date,NorthFork,col=cols[4],lwd=2,lty=1))
axis_fun(1,xmaj,xmin,format(xmaj,"%m/%d/%Y"),line=-0.5)
axis_fun(2,ymaj,ymin,format(ymaj),cex.axis=1)
box(lwd=1)
mtext("Date\n(MM/DD/YYYY)",side=1,line=2.75,cex=1)
...
```{r SLE salinity calendar,echo=FALSE,fig.width=6.5,fig.height=4.5,fig.align='center',fig.cap="St Lucie River Estuary salinity calendar plot." }
sal_cat <- seq(0,40,5)
...
mtext("Day of Water Year",side=1,line=2,cex=1)
mtext(expression("Cumulative Discharge (x1000 Ac-Ft d"^" -1"*")"),side=4,line=2.5,cex=1)
...
```{r LWL salinity calendar,echo=FALSE,fig.width=6.5,fig.height=4.5,fig.align='center',fig.cap="Lake Worth Lagoon salinity calendar plot." }
sal_cat <- seq(0,40,5)
